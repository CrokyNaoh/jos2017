...
trap_init(void)
{
	...
	//SETGATE(idt[3],1, GD_KT, t_brkpt, 0);
	...
	SETGATE(idt[9],0, GD_KT, t_gpflt, 0);
	...
	SETGATE(idt[15],0, GD_KT, t_gpflt, 0);
	...
	SETGATE(idt[IRQ_OFFSET+19], 0, GD_KT, irq_error, 0);
	// Per-CPU setup 
	trap_init_percpu();
	cprintf("trap init over\n");
}
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if(!(tf->tf_cs&0x3)) {
		...
		print_trapframe(tf);
		panic("This is a kernel page fault!\n");
	}
	...
	// LAB 4: Your code here.
	if(!curenv->env_pgfault_upcall) goto userfault;
